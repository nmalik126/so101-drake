...
When working with robot arms, accurate joint state estimation is crucial for executing precise trajectories. The motors on the SO-101 follwer arm (the Feetech STS3215 C001) have a 12-bit position encoder, yielding a theoretical position precision of less than 0.1 degrees. Unfortunately, when deployed in the SO-101 arm, some of the motors exhibit considerable encoder biases. For example, motor #3 associated with joint `elbow_flex` commonly exhibits a positive bias of over 3 degrees.

If the biases are left uncorrected, the SO-101 gripper's true position may end up several centimeters away from the commanded position, severely limiting task precision. The [official LeRobot calibration procedure](https://huggingface.co/docs/lerobot/so101#calibrate) attempts to address this, but unfortunately is not accurate enough (see [Visualization](#visualization)). This document introduces a revised motor calibration procedure for the SO-101 which yields considerably better (but not perfect) results.
