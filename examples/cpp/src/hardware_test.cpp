...
int main() {
    ...
    const Eigen::VectorXd q_closed { 
        Eigen::VectorXd::Map(
            constants::SO101_Q_INIT, 
            constants::SO101_NUM_Q
        ) 
    };
    Eigen::VectorXd q_open { q_closed };
    q_open(constants::SO101_NUM_Q - 1) = 0.5;
    const auto open_trajectory {
        PiecewisePolynomial<double>::FirstOrderHold(
            { 0.0, 2.0 },
            { q_closed, q_open }
        )
    };
    const auto close_trajectory {
        PiecewisePolynomial<double>::FirstOrderHold(
            { 0.0, 2.0 },
            { q_open, q_closed }
        )
    ...
    const auto trajectory {
        CompositeTrajectory<double>::AlignAndConcatenate({
            drake::copyable_unique_ptr<Trajectory<double>> { open_trajectory }, 
            drake::copyable_unique_ptr<Trajectory<double>> { close_trajectory },
        })
    };
